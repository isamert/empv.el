...
empv can browse and play audiobooks and podcast episodes from an [[https://www.audiobookshelf.org/][Audiobookshelf]] server. Configure the server URL and an API key (Audiobookshelf → Settings → API Keys):

#+begin_src elisp
(setq empv-audiobookshelf-url "https://books.example.com"
      empv-audiobookshelf-api-key "your-api-key")
#+end_src

Then use:

- ~empv-audiobookshelf-browse~ :: Pick a library, then a book or a podcast and one of its episodes.
- ~empv-audiobookshelf-search~ :: Search a library (live search when Consult is available).
- ~empv-audiobookshelf-continue-listening~ :: The server's /Continue Listening/ shelf.
- ~empv-audiobookshelf-recent-episodes~ :: Newest unfinished podcast episodes of a library.
- ~empv-audiobookshelf-sync-progress~ :: Send the current position to the server right now.

Playback resumes where you left off on any device. Progress is reported back every ~empv-audiobookshelf-progress-sync-interval~ seconds, when you pause, and when the item changes, so your phone and the web UI stay in sync. Multi-file audiobooks play as one continuous file and the server's chapters are available through ~empv-chapter-select~, ~empv-chapter-next~ and ~empv-chapter-prev~. Items can be bookmarked with ~empv-bookmark-set~ (through Embark, ~b~) and reopened with ~empv-bookmark-jump~. Loading with a resume position and chapters uses mpv's per-file options, which needs mpv 0.38 or newer.

Only episodes already downloaded to the server can be played; empv does not trigger downloads.
