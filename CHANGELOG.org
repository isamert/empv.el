...
- Add ~empv-youtube-show-video-details~, replacing the old ~empv-youtube-show-comments~. Now it also shows video metadata, thumbnail and automatically detects URL at point.
- Various bug fixes and stability improvements.
- README now contains a section about useful mpv extensions that works well with empv, check them out!

* 6.1.0

- Drop ~fd~ dependency for discovering media files.
- ~empv-play-directory~ now supports recursively playing given directory.

* 6.0.0

- Now you can use *YouTube* functionality without using an Invidious instance. You need to do ~(setq empv-invidious-instance 'ivjs)~ to be able to use. See [[file:README.org::*General YouTube configuration][README's YouTube configuration section]].
- Create any nonexistent parent dirs in ~empv-youtube-download~ (thanks to @QiangF!).
- Various bug fixes.
